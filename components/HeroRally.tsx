...
const KEYFRAME_ANGLES: KeyframeAngles[] = [
  ...
  // clip), extracted via RTMPose, double-smoothed, and capped:
  ...
  //      - smooths each joint trajectory (5-frame moving avg),
  //        then derives + smooths angles (3-frame on unwrapped),
  //      - racket angle = SMOOTHED forearm direction with a fixed
  //        0.10 length,
  //      - mirrors angles around the y-axis (right figure hits
  //        right-to-left),
  //      - caps fArmR's upward excursion (forearm rotation pulled
  //        50%% toward nearest horizontal axis) and uArmR's
  //        horizontal-LEFT reach (excess past 145 deg unwrapped
  //        scaled to 30%%) so the racket tip during follow-through
  //        stays at shoulder height instead of sweeping above the
  //        head. Preserves the cross-body sweep direction.
  //   3. Window: 1.0s pre-contact + 0.55s post (47 frames).
